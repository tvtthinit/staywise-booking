...
function App() {
  ...
  async function loadHistory() {
    if (!email) return
    setHistory(await getBookingHistory(email))
    setWorkspace('trips')
  }
  ...
  return (
    <main className={selectedHotel ? 'detail-mode' : ''}>
      <nav className="nav shell">
        <a className="brand" href="/" onClick={(event) => { if (selectedHotel) { event.preventDefault(); closeHotel() } }}>stay<span>wise</span></a>
        <div className="nav-links"><a href="#stays">Explore stays</a><a href="#why">Why Staywise</a><button onClick={loadHistory}>My trips</button><button onClick={() => setWorkspace('profile')}>Profile</button></div>
      ...
      <form className="search-panel shell" onSubmit={handleSearch}>
      ...
      <section className="stays shell" id="stays">
